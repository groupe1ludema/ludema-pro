...
<?php //var_dump($description)?>
	...
	<form class=" offset-lg-3 col-lg-6 formultest" action='?controller=genererpdf' method='post'>
		...
		<?php elseif($testAndQ['Inputtype']=="commentaire") :?>  <!-- on affiche une grande zone de texte -->
				<div class="form-group">
				<textarea class="form-control"name=<?= e($name);?> rows="2" cols="28"></textarea>
				</div>
    ...
    <?php elseif($testAndQ['Inputtype']=="axe17") :?> <!-- Ici on affiche un axe graduée de 1 à 7 -->
  			<div class="form-group">
  			<input type="range" class="slider form-control" name=<?= e($name);?> min="1" max="7" step="1" value="0" oninput="<?=e($name).'res'?>.value=parseInt(<?= e($name);?>.value)"/>
  			<output name=<?= e($name)."res"?>>1</output>
  			</div>
		<?php endif?>
	<?php endforeach?>
<?php endfor?>
